...
app.post('/add-blog',isLoggedInOrNot ,async (req, res) => {
    ...
    const { Title, subtitle,description } = req.body;
     ...
     console.log('New Blog:', Title, RTCSessionDescription, subtitle);
...
app.get("/edit/:id",isLoggedInOrNot,async(req,res) => {
   const id = req.params.id
   const todos = await db.blogs.findAll({
    where: {
        id:id
    }
   }) 
//    res.send(todos) //sending the todo data to the client
   ...
   res.render("todo/edit-blog", {blogs: blogs}) //rendering the edit-todo.ejs file and passing the todo data to it
})

app.post("/edit-blog/:id", isLoggedInOrNot,async (req, res) => {
    const id = req.params.id
